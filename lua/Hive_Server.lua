...
local function SpawnEgg(self)
    ...
    //let eggs spawn on normal ground (ISSUE #2)
...
function Hive:GenerateEggSpawns(hiveLocationName)
    ...
    for index = 1, maxAttempts do
        ...
        if powerPoints == nil or #powerPoints == 0 then
            spawnPoint = GetRandomSpawnForCapsule(capsuleHeight, capsuleRadius, self:GetModelOrigin(), kEggMinRange, kEggMaxRange, EntityFilterAll())
        else
            local c_powerPoint = powerPoints[math.random( #powerPoints )]
            spawnPoint = GetRandomSpawnForCapsule(capsuleHeight, capsuleRadius, c_powerPoint:GetOrigin(), kEggMinRange, kEggMaxRange, EntityFilterAll())
        end
